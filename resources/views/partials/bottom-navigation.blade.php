<div id="bottom-navigation"
    ...
    <div class="grid h-full max-w-lg grid-cols-4 mx-auto font-medium">
        <x-bottom-nav-link href="{{ route('dashboard') }}" :active="request()->routeIs('dashboard')">
            ...
            <span class="text-sm group-hover:text-teal-700">Home</span>
        </x-bottom-nav-link>
        <x-bottom-nav-link href="{{ route('kelas.index') }}" :active="request()->routeIs('kelas.index')">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                class="w-5 h-5 mb-2 bi bi-book-half group-hover:text-teal-700 " viewBox="0 0 16 16">
                <path
                    d="M8.5 2.687c.654-.689 1.782-.886 3.112-.752 1.234.124 2.503.523 3.388.893v9.923c-.918-.35-2.107-.692-3.287-.81-1.094-.111-2.278-.039-3.213.492V2.687zM8 1.783C7.015.936 5.587.81 4.287.94c-1.514.153-3.042.672-3.994 1.105A.5.5 0 0 0 0 2.5v11a.5.5 0 0 0 .707.455c.882-.4 2.303-.881 3.68-1.02 1.409-.142 2.59.087 3.223.877a.5.5 0 0 0 .78 0c.633-.79 1.814-1.019 3.222-.877 1.378.139 2.8.62 3.681 1.02A.5.5 0 0 0 16 13.5v-11a.5.5 0 0 0-.293-.455c-.952-.433-2.48-.952-3.994-1.105C10.413.809 8.985.936 8 1.783z" />
            </svg>
            <span class="text-sm group-hover:text-teal-700">Kelas</span>
        </x-bottom-nav-link>

        <x-bottom-nav-link href="{{ route('nilai-rapor.index') }}" :active="request()->routeIs('nilai-rapor.*')"
            class="inline-flex flex-col items-center justify-center px-5 hover:bg-gray-50 dark:hover:bg-gray-800 group">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                class="w-5 h-5 mb-2 bi bi-clipboard-check-fill group-hover:text-teal-700" viewBox="0 0 16 16">
                <path
        ...
        <x-bottom-nav-link href="{{ route('profile.show') }}" :active="request()->routeIs('profile.*')">
            ...
            <span class="text-sm  group-hover:text-teal-600 ">Profile</span>
        </x-bottom-nav-link>
    </div>
</div>